...
// WebGL Resource Manager for memory leak prevention
class WebGLResourceManager {
  constructor(gl) {
    ...
    this.transformFeedbacks = new Set();
  }
  ...
  deleteTransformFeedback(tf) {
    if (this.transformFeedbacks.has(tf)) {
  ...
  cleanup() {
    ...
    this.transformFeedbacks.forEach(tf => this.gl.deleteTransformFeedback(tf));
  ...
  getResourceCount() {
    return {
      ...
      transformFeedbacks: this.transformFeedbacks.size
    };
  }
}
...
class ShaderCache {
  ...
  generateKey(vertexSource, fragmentSource) {
    return `${vertexSource.length}_${fragmentSource.length}_${this.hashCode(vertexSource)}_${this.hashCode(fragmentSource)}`;
  }
  ...
  // Get linked program from cache
  getProgram(key) {
    return this.programs.get(key);
  }

  // Store linked program in cache
  storeProgram(key, program) {
  ...
  getStats() {
    return {
      shaders: this.cache.size,
      programs: this.programs.size,
      totalItems: this.cache.size + this.programs.size
    };
  }
}
...
// Texture Pool for memory optimization
class TexturePool {
  constructor(gl) {
    this.gl = gl;
    this.availableTextures = new Map(); // format -> [textures]
    this.inUseTextures = new Set();
    this.maxPoolSize = 50; // Maximum textures to keep in pool
  }

  // Get a texture from pool or create new one
  getTexture(format, width, height, filter = this.gl.LINEAR) {
    const key = `${format}_${width}_${height}_${filter}`;
    
    // Check if we have an available texture
    if (this.availableTextures.has(key) && this.availableTextures.get(key).length > 0) {
      const texture = this.availableTextures.get(key).pop();
  ...
  returnTexture(texture, format, width, height, filter = this.gl.LINEAR) {
    if (!this.inUseTextures.has(texture)) {
      return; // Texture not from this pool
    }
    ...
    // Limit pool size
    if (this.availableTextures.get(key).length < this.maxPoolSize) {
  ...
  // Clear all textures in pool
  clear() {
    this.availableTextures.forEach(textures => {
  ...
  getStats() {
    ...
    return {
      ...
      inUse: this.inUseTextures.size,
      total: totalAvailable + this.inUseTextures.size
    };
  }
}
...
// Batch Rendering System for performance optimization
class BatchRenderer {
  constructor(gl) {
    this.gl = gl;
    this.batches = new Map(); // program -> [render calls]
    this.currentBatch = null;
    this.batchSize = 100; // Maximum calls per batch
  }

  // Start a new batch
  beginBatch(program) {
    if (this.currentBatch && this.currentBatch.program === program) {
      return; // Continue current batch
    }
  ...
  // Add a render call to current batch
  addCall(renderCall) {
    if (!this.currentBatch) {
      console.warn('No active batch, call beginBatch first');
      return;
    }
    
    this.currentBatch.calls.push(renderCall);
    
    // Flush if batch is full
    if (this.currentBatch.calls.length >= this.batchSize) {
      this.flushBatch();
    }
  }

  // Execute all calls in current batch
  flushBatch() {
    if (!this.currentBatch || this.currentBatch.calls.length === 0) {
      return;
    }
    
    // Use the program
    this.gl.useProgram(this.currentBatch.program);
    
    // Execute all render calls
    this.currentBatch.calls.forEach(call => {
      call();
    });
    
    // Clear batch
    this.currentBatch.calls = [];
  }
  ...
  getStats() {
    ...
    return {
      ...
      currentBatchSize: this.currentBatch ? this.currentBatch.calls.length : 0
    };
  }
}
...
class PerformanceProfiler {
  ...
  measure(name, startMark, endMark = null) {
    ...
    if (start && end) {
      ...
      // Limit history size
      if (this.frameData.length > this.maxFrameHistory) {
  ...
  getAverageDuration(name, frames = 60) {
    const recentMeasures = this.frameData
      .filter(data => data.name === name)
      .slice(-frames);
    
    if (recentMeasures.length === 0) return 0;
    
    const total = recentMeasures.reduce((sum, data) => sum + data.duration, 0);
  ...
  getReport() {
    ...
    const measureNames = [...new Set(this.frameData.map(data => data.name))];
    
    measureNames.forEach(name => {
      const average = this.getAverageDuration(name);
      const recent = this.getAverageDuration(name, 10);
      
      report.measures[name] = {
        current: this.measures.get(name) || 0,
        average: average,
        recent: recent,
        trend: recent > average ? 'increasing' : recent < average ? 'decreasing' : 'stable'
      };
      
      // Identify bottlenecks
      if (average > 16.67) { // More than 1/60th of a second
        report.bottlenecks.push({
          name,
          duration: average,
          impact: average / 16.67
        });
      }
    });
    ...
    if (report.bottlenecks.length > 0) {
      report.bottlenecks.sort((a, b) => b.impact - a.impact);
      
      report.bottlenecks.forEach(bottleneck => {
        if (bottleneck.name.includes('precipitation')) {
          report.recommendations.push(`Reduce precipitation particles or enable LOD for ${bottleneck.name}`);
        } else if (bottleneck.name.includes('render')) {
          report.recommendations.push(`Enable batch rendering or reduce rendering quality for ${bottleneck.name}`);
        } else if (bottleneck.name.includes('simulation')) {
          report.recommendations.push(`Reduce simulation iterations or resolution for ${bottleneck.name}`);
        }
      });
    }
  ...
  getGPUMemoryUsage() {
    if (gl && gl.getParameter) {
      try {
        // This is not standard WebGL, but some browsers support it
...
const guiControls_default = {
  ...
  enhancedWaterDepth : true, // Enable enhanced water depth visualization
...
// Precipitation LOD system for performance optimization
class PrecipitationLOD {
  constructor() {
    this.baseDivider = 25;
    this.minParticles = 1000;
    this.maxParticles = 100000;
    this.zoomThresholds = [0.5, 1.0, 2.0, 5.0, 10.0];
    this.zoomMultipliers = [0.1, 0.25, 0.5, 0.75, 1.0];
    this.performanceMultiplier = 1.0;
  }

  calculateParticleCount(simResX, simResY, zoom, fps) {
    // Base calculation
    let baseCount = (simResX * simResY) / this.baseDivider;
    
    // Apply zoom-based LOD
    let zoomMultiplier = 1.0;
    for (let i = 0; i < this.zoomThresholds.length; i++) {
      if (zoom <= this.zoomThresholds[i]) {
        zoomMultiplier = this.zoomMultipliers[i];
        break;
      }
    }
    ...
    if (fps < 30) {
      this.performanceMultiplier = Math.max(0.1, this.performanceMultiplier * 0.9);
    } else if (fps > 55) {
      this.performanceMultiplier = Math.min(1.0, this.performanceMultiplier * 1.05);
    }
    
    // Calculate final count with constraints
    let finalCount = Math.round(baseCount * zoomMultiplier * this.performanceMultiplier);
    finalCount = Math.max(this.minParticles, Math.min(this.maxParticles, finalCount));
    
    return finalCount;
  }

  updatePerformanceMultiplier(fps) {
    if (fps < 25) {
      this.performanceMultiplier = Math.max(0.1, this.performanceMultiplier * 0.8);
    } else if (fps < 40) {
      this.performanceMultiplier = Math.max(0.1, this.performanceMultiplier * 0.9);
    } else if (fps > 55) {
      this.performanceMultiplier = Math.min(1.0, this.performanceMultiplier * 1.02);
    }
  }
}

// Global precipitation LOD instance (temporarily disabled for debugging)
precipitationLOD = new PrecipitationLOD();
...
class FBO // wraps texture, frambuffer and info in one
{
  ...
  constructor(w, h, internalFormat, format, type, texFilter, wrapMode_S)
  {
    ...
    this.texture = texturePool ? texturePool.getTexture(internalFormat, w, h, texFilter) : resourceManager.createTexture();
    ...
    this.frameBuffer = resourceManager.createFramebuffer();
...
async function loadData()
{
  ...
  if (file) {                                                    // load data from save file
    ...
    if (version == saveFileVersionID || version == 1939327491) { // also allow previous version, settings will not be loaded
      ...
      // Use LOD system for dynamic particle count (temporarily disabled for debugging)
      const currentFPS = performanceMonitor ? performanceMonitor.getAverageFPS() : 60;
      const currentZoom = cam ? cam.curZoom : 1.0;
      NUM_DROPLETS = precipitationLOD.calculateParticleCount(sim_res_x, sim_res_y, currentZoom, currentFPS);
    ...
    // Use LOD system for dynamic particle count (temporarily disabled for debugging)
    const currentFPS = performanceMonitor ? performanceMonitor.getAverageFPS() : 60;
    const currentZoom = 1.0; // Default zoom for new simulations
    NUM_DROPLETS = precipitationLOD.calculateParticleCount(sim_res_x, sim_res_y, currentZoom, currentFPS);
    SETUP_MODE = true;
...
async function mainScript(initialBaseTex, initialWaterTex, initialWallTex, initialRainDrops)
{
  ...
  class SoundSystem
  {
    ...
    constructor()
    {
      ...
      this.loadSound('urban.m4a').then(buffer => { 
        this.urban_sound = this.playLoop(buffer, 0.0);
        this.urban_sound.baseVolume = 0.0;
      });
      this.loadSound('forest.mp3').then(buffer => { 
        this.forest_sound = this.playLoop(buffer, 0.0);
        this.forest_sound.baseVolume = 0.0;
      });
      this.loadSound('beach.mp3').then(buffer => { 
        this.beach_sound = this.playLoop(buffer, 0.0);
        this.beach_sound.baseVolume = 0.0;
      });
      this.loadSound('rain.m4a').then(buffer => { 
        this.rain_sound = this.playLoop(buffer, 0.0);
        this.rain_sound.baseVolume = 0.0;
      });
      this.loadSound('wind.m4a').then(buffer => { 
        this.wind_sound = this.playLoop(buffer, 0.0);
        this.wind_sound.baseVolume = 0.0;
      });
    }
    ...
    setSoundGainAndPan(sound, gain, pan = 0.0)
    {
      if (sound) {
        ...
        let finalGain = gain * this.ambientVolume * this.masterVolume;
        sound.gain.value = finalGain;
        sound.pan.value = pan;
        sound.baseVolume = gain; // Store the base volume for later updates
      }
    }
    ...
    updateAllVolumes()
    {
      // Update ambient sound volumes with new master and ambient volume settings
      if (this.forest_sound) {
        this.forest_sound.gain.value = this.forest_sound.baseVolume * this.ambientVolume * this.masterVolume;
      }
      if (this.beach_sound) {
        this.beach_sound.gain.value = this.beach_sound.baseVolume * this.ambientVolume * this.masterVolume;
      }
      if (this.urban_sound) {
        this.urban_sound.gain.value = this.urban_sound.baseVolume * this.ambientVolume * this.masterVolume;
      }
      if (this.rain_sound) {
        this.rain_sound.gain.value = this.rain_sound.baseVolume * this.ambientVolume * this.masterVolume;
      }
      if (this.wind_sound) {
        this.wind_sound.gain.value = this.wind_sound.baseVolume * this.ambientVolume * this.masterVolume;
      }
    }
  }
  ...
  // Initialize resource manager and performance monitor (temporarily disabled for debugging)
  ...
  function setupDatGui(strGuiControls)
  {
    ...
    radiation_folder.add(guiControls, 'latitude', -90.0, 90.0, 0.1).onChange(function() { 
      updateSunlight(); 
      // Update Coriolis effect for new latitude
      gl.useProgram(pressureProgram);
      gl.uniform1f(gl.getUniformLocation(pressureProgram, 'latitude'), guiControls.latitude);
    }).name('Latitude').listen();
    ...
    water_folder.add(guiControls, 'enhancedWaterDepth').name('Enhanced Water Depth').onChange(function() {
      // This will be handled in the shader - the shader checks this uniform
      gl.useProgram(realisticDisplayProgram);
      gl.uniform1i(gl.getUniformLocation(realisticDisplayProgram, 'enhancedWaterDepth'), guiControls.enhancedWaterDepth ? 1 : 0);
    });

    // Atmospheric Physics Controls
    var atmospheric_folder = datGui.addFolder('Atmospheric Physics');
    
    atmospheric_folder.add(guiControls, 'coriolisStrength', 0.0, 2.0, 0.1).name('Coriolis Effect Strength').onChange(function() {
      gl.useProgram(velocityProgram);
      gl.uniform1f(gl.getUniformLocation(velocityProgram, 'coriolisStrength'), guiControls.coriolisStrength);
    });
    
    atmospheric_folder.add(guiControls, 'pressureGradientStrength', 0.0, 2.0, 0.1).name('Pressure Gradient Strength').onChange(function() {
      gl.useProgram(velocityProgram);
      gl.uniform1f(gl.getUniformLocation(velocityProgram, 'pressureGradientStrength'), guiControls.pressureGradientStrength);
    });
    ...
    advanced_folder.add(guiControls, 'masterVolume', 0, 2, 0.1).name('Master Volume').onChange(function() {
      if (soundSystem) {
        soundSystem.setMasterVolume(guiControls.masterVolume);
      }
    });
    
    advanced_folder.add(guiControls, 'ambientVolume', 0, 2, 0.1).name('Ambient Volume').onChange(function() {
      if (soundSystem) {
        soundSystem.setAmbientVolume(guiControls.ambientVolume);
      }
    });
    
    advanced_folder.add(guiControls, 'thunderVolume', 0, 2, 0.1).name('Thunder Volume').onChange(function() {
      if (soundSystem) {
        soundSystem.setThunderVolume(guiControls.thunderVolume);
      }
    });
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.code == 'ControlRight') {
      ...
      // F12: toggle performance display
  ...
  var fluidVao = resourceManager.createVertexArray(); // vertex array object to store
  // bufferData and vertexAttribPointer
  gl.bindVertexArray(fluidVao);
  var fluidVertexBufferObject = resourceManager.createBuffer();
  ...
  var postProcessingVao = resourceManager.createVertexArray(); // vertex array object to store
  // bufferData and vertexAttribPointer
  gl.bindVertexArray(postProcessingVao);
  var postProcessingVertexBufferObject = resourceManager.createBuffer();
  ...
  const precipitationVao_0 = resourceManager.createVertexArray();
  const precipVertexBuffer_0 = resourceManager.createBuffer();
  const precipitationTF_0 = resourceManager.createTransformFeedback();
  const precipitationVao_1 = resourceManager.createVertexArray();
  const precipVertexBuffer_1 = resourceManager.createBuffer();
  const precipitationTF_1 = resourceManager.createTransformFeedback();
  ...
  */
  ...
  const baseTexture_0 = resourceManager.createTexture();
  const baseTexture_1 = resourceManager.createTexture();
  const waterTexture_0 = resourceManager.createTexture();
  const waterTexture_1 = resourceManager.createTexture();
  const wallTexture_0 = resourceManager.createTexture();
  const wallTexture_1 = resourceManager.createTexture();

  const curlTexture = resourceManager.createTexture();
  const vortForceTexture = resourceManager.createTexture();

  const lightTexture_0 = resourceManager.createTexture();
  const lightTexture_1 = resourceManager.createTexture();
  const precipitationFeedbackTexture = resourceManager.createTexture();
  const precipitationDepositionTexture = resourceManager.createTexture();
  const lightningDataTexture = resourceManager.createTexture(); // single pixel texture holding location and timing of current lightning strike

  // Static texures:
  const noiseTexture = resourceManager.createTexture();
  const A380Texture = resourceManager.createTexture();
  const A380GearTexture = resourceManager.createTexture();
  const surfaceTextureMap = resourceManager.createTexture();
  const colorScalesTexture = resourceManager.createTexture();

  const lightningTextures = [];
  const numLightningTextures = 10;


  frameBuff_0 = resourceManager.createFramebuffer(); // global for weather stations
  const frameBuff_1 = resourceManager.createFramebuffer();

  const curlFrameBuff = resourceManager.createFramebuffer();
  const vortForceFrameBuff = resourceManager.createFramebuffer();

  lightFrameBuff_0 = resourceManager.createFramebuffer();
  const lightFrameBuff_1 = resourceManager.createFramebuffer();
  const precipitationFeedbackFrameBuff = resourceManager.createFramebuffer();
  const lightningDataFrameBuff = resourceManager.createFramebuffer();
  ...
  {
    lightningTextures[i] = resourceManager.createTexture();
  ...
  gl.uniform4fv(gl.getUniformLocation(advectionProgram, 'realWorldSounding_Velv'), realWorldSounding_Vel);
    ...
    // Atmospheric Physics uniforms
    gl.uniform1f(gl.getUniformLocation(velocityProgram, 'latitude'), guiControls.latitude);
    gl.uniform1f(gl.getUniformLocation(velocityProgram, 'coriolisStrength'), guiControls.coriolisStrength);
    gl.uniform1f(gl.getUniformLocation(velocityProgram, 'pressureGradientStrength'), guiControls.pressureGradientStrength);
  ...
  { // Runs for every frame
    performanceProfiler.mark('draw_start');
    batchRenderer.beginBatch(null); // Start batch for this frame
    ...
    if (SETUP_MODE) {
      ...
      // User input processing - always active, even when paused
      // Note: User input is processed but not applied to textures when paused
      // This prevents breaking the simulation while still allowing tool selection and cursor display
        ...
        if (!airplaneMode || airplane.hasCrashed() || frameNum % 17 == 0) { // update every 17 frames because 60 * 0.288 secs per iteration = 17.28
          ...
          for (var i = 0; i < numIterations; i++) { // Simulation loop
            ...
            if (guiControls.enablePrecipitation) { // move precipitation, HUGE PERFORMANCE BOTTLENECK!
              ...
              // sample to count number of inactive droplets (optimized frequency)
              if (IterNum % 1200 == 0) { // Reduced frequency from 600 to 1200
                gl.readBuffer(gl.COLOR_ATTACHMENT0);
    ...
    frameNum++;
    batchRenderer.end(); // End batch for this frame
    performanceProfiler.measure('draw_frame', 'draw_start');
    requestAnimationFrame(draw);
  }
  ...
  {
    // Generate cache key for program (temporarily disabled to fix WebGL warnings)
    /*
    const tfKey = transform_feedback_varyings ? '_tf' : '';
    const cacheKey = `program_${vertexShader}_${fragmentShader}${tfKey}_fb`;
    
    // Check program cache first
    if (shaderCache.hasProgram(cacheKey)) {
      console.log('Program cache hit');
      return shaderCache.getProgram(cacheKey);
    }
    */
    ...
    if (gl.getProgramParameter(program, gl.LINK_STATUS)) {
      // Store in cache (temporarily disabled to fix WebGL warnings)
      // shaderCache.storeProgram(cacheKey, program);
  ...
  async function loadShader(nameIn)
  {
    ...
    const cacheKey = shaderCache.generateKey(shaderSource, '');
    if (shaderCache.hasShader(cacheKey)) {
      console.log('Shader cache hit for: ' + nameIn);
      return new Promise(async (resolve) => {
        await loadingBar.add(1, 'Loading cached shader: ' + nameIn);
        resolve(shaderCache.getShader(cacheKey));
      });
    }
    ...
    // Store in cache
    shaderCache.storeShader(cacheKey, shader);
  ...
  function updatePerformanceDisplay() {
    if (!performanceDisplay || !performanceMonitor) return;
    
    const report = performanceMonitor.getPerformanceReport();
    const resources = resourceManager ? resourceManager.getResourceCount() : null;
    const shaderStats = shaderCache ? shaderCache.getStats() : null;
    const textureStats = texturePool ? texturePool.getStats() : null;
    const profilerReport = performanceProfiler ? performanceProfiler.getReport() : null;
    
    let displayText = `FPS: ${report.avgFPS.toFixed(1)}\n`;
    displayText += `Frame Time: ${report.avgFrameTime.toFixed(1)}ms\n`;
    displayText += `Variance: ${report.fpsVariance.toFixed(1)}\n`;
    displayText += `Particles: ${NUM_DROPLETS.toLocaleString()}\n`;
    
    if (resources) {
      displayText += `Textures: ${resources.textures}\n`;
      displayText += `Framebuffers: ${resources.framebuffers}\n`;
      displayText += `Buffers: ${resources.buffers}\n`;
    }
    
    if (shaderStats) {
      displayText += `Shaders: ${shaderStats.shaders}\n`;
      displayText += `Programs: ${shaderStats.programs}\n`;
    }
    
    if (textureStats) {
      displayText += `Tex Pool: ${textureStats.inUse}/${textureStats.total}\n`;
    }
    
    if (profilerReport && profilerReport.bottlenecks.length > 0) {
      const topBottleneck = profilerReport.bottlenecks[0];
      displayText += `Bottleneck: ${topBottleneck.name}\n`;
    }
    
    if (cam) {
      displayText += `Zoom: ${cam.curZoom.toFixed(2)}x\n`;
    }
    
    performanceDisplay.textContent = displayText;
  }

  function togglePerformanceDisplay() {
    if (performanceDisplay) {
      const isVisible = performanceDisplay.style.display !== 'none';
      performanceDisplay.style.display = isVisible ? 'none' : 'block';
      if (!isVisible) {
  ...
  {
    if (!isPageHidden()) {
      const currentTime = performance.now(); // Temporarily disabled for debugging
      ...
      // Update performance monitor (temporarily disabled for debugging)
      performanceMonitor.updateFPS(FPS);
      performanceMonitor.updateFrameTime(frameTime);
      
      // Update precipitation LOD based on performance (temporarily disabled for debugging)
      if (precipitationLOD) {
        precipitationLOD.updatePerformanceMultiplier(FPS);
      }

      if (!guiControls.paused) {
        const report = performanceMonitor.getPerformanceReport(); // Temporarily disabled for debugging
        console.log(FPS + ' FPS   ' + guiControls.IterPerFrame + ' Iterations / frame      ' + FPS * guiControls.IterPerFrame + ' Iterations / second');
        console.log('Avg FPS: ' + report.avgFPS.toFixed(1) + ' | Frame Time: ' + report.avgFrameTime.toFixed(1) + 'ms | Variance: ' + report.fpsVariance.toFixed(1));
        
        // Update performance display
        if (performanceDisplay && performanceDisplay.style.display !== 'none') {
          updatePerformanceDisplay();
        }
