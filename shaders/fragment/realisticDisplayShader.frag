...
uniform int enhancedWaterDepth;
...
void main()
{
  ...
  if (texCoord.y < 0.) { // < texelSize.y below simulation area
    ...
    case WALLTYPE_LAND:
      ...
      break;
    case WALLTYPE_WATER:
      if (enhancedWaterDepth == 1) {
        // Enhanced water depth visualization
        float waterDepth = float(-wall[VERT_DISTANCE]) - fragCoord.y;
        
        // Get underwater terrain color (what would be visible if water was clear)
        ivec4 wallXmY0 = texture(wallTex, texCoordXmY0);
        ivec4 wallXpY0 = texture(wallTex, texCoordXpY0);
        float interpDepth = mix(mix(float(-wallXmY0[VERT_DISTANCE]), float(-wall[VERT_DISTANCE]), clamp(fract(fragCoord.x) + 0.5, 0.5, 1.)), float(-wallXpY0[VERT_DISTANCE]), clamp(fract(fragCoord.x) - 0.5, 0., 0.5));
        float terrainDepth = interpDepth - fract(fragCoord.y);
        
        vec3 underwaterTerrain = getWallColor(terrainDepth);
        
        // Water color based on depth
        vec3 shallowWater = vec3(0.0, 0.7, 1.0);  // Light blue for shallow water
        vec3 deepWater = vec3(0.0, 0.2, 0.6);     // Dark blue for deep water
        
        // Depth factor (0 = shallow, 1 = deep)
        ...
        // Add underwater terrain visibility (more visible in shallow water)
        float terrainVisibility = max(0.0, 1.0 - depthFactor * 2.0);
        waterColor = mix(underwaterTerrain * 0.6, waterColor, 1.0 - terrainVisibility * 0.3);
        
        // Add water surface reflection and waves
        float waveTime = iterNum * 0.01;
        float waveX = sin(fragCoord.x * 0.1 + waveTime) * 0.02;
        float waveY = cos(fragCoord.y * 0.05 + waveTime * 0.7) * 0.01;
        float waveEffect = (waveX + waveY) * (1.0 - depthFactor);
        
        // Add subtle wave highlights
        vec3 waveHighlight = vec3(0.1, 0.2, 0.3) * waveEffect;
        waterColor += waveHighlight;
        
        // Add depth-based transparency
        float waterTransparency = 0.8 + depthFactor * 0.2;
        
        color = waterColor;
        opacity = waterTransparency;
      } else {
        // Simple water rendering (original)
